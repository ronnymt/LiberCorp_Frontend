...
import { initNetworkAlerts } from './network-alerts.js';
import { logout, getUsuario } from '../lib/auth.js';
...
function bindNavSubmenus() {
  ...
  // The group containing the current page always wins -- otherwise landing on
  // a page (via link, bookmark, or reload) could leave its own group collapsed
  // behind whatever the user had open on a previous, unrelated page. The
  // remembered/stored group is only a fallback for pages with no active group
  // at all (e.g. cliente.html, reached from Clientes but without its own NAV
  // entry), so the sidebar doesn't just collapse everything.
  const activeIndex = trees.findIndex((t) => t.classList.contains('has-active'));
  if (activeIndex !== -1) {
    setStoredOpenIndex(activeIndex);
  } else {
...
function openSignOutModal() {
  showModal({
    title: 'Cerrar sesion?',
    size: 'sm',
    body: '<p style="font-size:13px;color:var(--text-secondary);line-height:1.6;margin:0">Vas a tener que volver a iniciar sesion para acceder al panel.</p>',
    actions: [
      { label: 'Cancelar', variant: 'ghost' },
      {
        label: 'Cerrar sesion',
        variant: 'primary',
        // logout() invalida el token en el backend (ver AuthService.logout) y
        // recien despues limpia localStorage/redirige -- antes este modal era
        // el demo original del template, solo redirigia sin borrar nada.
        action: () => { logout(); }
      }
    ]
  });
...
/**
 * Items marcados `hidden: true` se dejan definidos (con su accion intacta,
 * por si se reactivan despues) pero no se renderizan -- pedido explicito del
 * usuario 2026-08-28: solo "Profile" y "Cerrar sesion" visibles por ahora.
 * Ver visibleUserMenuItems().
 */
const USER_MENU = [
  { label: 'Profile',            action: () => { window.location.href = 'profile.html'; } },
  { label: 'Account settings',   action: () => { window.location.href = 'settings.html'; }, hidden: true },
  { label: 'Theme generator',    action: () => { window.location.href = 'theme.html'; }, hidden: true },
  { label: 'Keyboard shortcuts', action: openShortcutsModal, hidden: true },
  { label: 'Help & support',     action: () => { window.location.href = 'faq.html'; }, hidden: true },
  { label: 'Lock screen',        action: () => { window.location.href = 'lock_screen.html'; }, hidden: true },
  { label: 'Cerrar sesion',      action: openSignOutModal, variant: 'danger', icon: ICONO_LOGOUT }
];

/** Filtra los items ocultos y reinserta un separador entre cada uno visible que quede -- con solo Profile + Cerrar sesion da exactamente un divisor entre ambos. */
function visibleUserMenuItems() {
  const visibles = USER_MENU.filter((item) => item !== '-' && !item.hidden);
  return visibles.flatMap((item, i) => (i === 0 ? [item] : ['-', item]));
}
...
function bindTopbarPanels() {
  ...
  const account = document.querySelector('.tb-account');
  if (account) {
    account.addEventListener('click', (e) => {
      e.preventDefault(); e.stopPropagation();
      openMenu(account, visibleUserMenuItems());
    });
  }
}

/** Nombre/rol reales del usuario logueado (ver getUsuario() en lib/auth.js) -- rellena el trigger del menu de cuenta en el topbar, que se renderiza vacio porque shell-render.js no tiene acceso a localStorage en build-time. */
function applyAccountInfo() {
  const usuario = getUsuario();
  if (!usuario) {return;}
  const nameEl = document.querySelector('.tb-account-info .name');
  const roleEl = document.querySelector('.tb-account-info .role');
  ...
  if (avatarEl) {avatarEl.textContent = (usuario.nombre ?? '?').trim().charAt(0).toUpperCase();}
}
