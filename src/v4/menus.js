...
function buildMenu(items) {
  ...
  items.forEach((item) => {
    ...
    btn.type = 'button';
    btn.className = item.variant ? `menu-item menu-item-${item.variant}` : 'menu-item';
    btn.setAttribute('role', 'menuitem');
    if (item.icon) {
      const iconSpan = document.createElement('span');
      iconSpan.className = 'menu-item-icon';
      iconSpan.innerHTML = item.icon; // solo viene de codigo propio (SVG fijo), nunca de input del usuario
...
/**
 * @typedef {{ label: string, icon?: string, variant?: 'danger', action?: () => void }} MenuItem
 *   A single menu entry. If `action` is omitted, clicking it shows a toast with the label.
 *   `icon`, if given, is a trusted inline SVG string (16x16, stroke="currentColor") — never pass user input here.
 *   `variant: 'danger'` styles it red (destructive/sign-out actions).
...
// capture:true so this sees the underlying page scrolling out from under an
// open popover -- but scroll doesn't bubble, so without the contains() guard
// it also fires for scrolling *inside* the popover itself (e.g. .panel-list's
// own overflow-y: auto), closing it before the user can scroll the list.
window.addEventListener('scroll', (e) => {
  // e.target is `window` (not a Node) when the scroll fires on the window
  // itself rather than a specific element -- contains() throws on that.
